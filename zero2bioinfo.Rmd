...
author: "Jonathan Dreyfuss, Assoc Dir, Bioinfo & Biostat Core, Joslin Diabetes Center"
date: "`r Sys.Date()`"
...
# Welcome
You'd like to analyze and visualize your data on a computer. Your workflow will be the most flexible, powerful, and reproducible if it involves *programming*. Programming looks intimidating, but once you understand the concepts, you should be able to Google questions, understand the answers, and be productive. That's the goal of this tutorial.

Once you decide to program, you need to decide which software to use. There's a tremendous amount of software development ongoing in this field, now termed *data science*, particularly in the free languages [R](https://www.r-project.org/) and [Python](https://www.python.org/). In academia, an [analysis](http://r4stats.com/articles/popularity/) of statistical software in Google Scholar papers shows that **R** is currently the 2nd most used software, and is overtaking other languages. (In case you're interested, the author of this analysis also wrote [R for SAS and SPSS Users](http://r4stats.com/books/r4sas-spss/).) This mirrors the general trend where free programming languages dominate in popularity across computing applications.

![*The Popularity of Data Science Software*, Fig 2d](Fig_2d_ScholarlyImpact2016.png)

# R
- R is a free language and software for statistical computing and graphics  
- R has thousands of free contributed packages, such as roughly 1000 bioinformatics packages that make up the [Bioconductor](https://www.bioconductor.org/) project  
...
Dr. Hui Pan (Bioinfo & Biostat Core) and I have developed several R packages to streamline bioinformatics analysis. These are based on the popular R package [limma](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4402510/), which uses linear modeling of omics data with sophisticated variance estimation to improve power, and has been validated multiple times. Our packages are freely available on the popular code repository [Github](https://github.com/). R packages on Github can be directly installed in R. Two of our packages that we'll go over today are [ezlimma](https://github.com/jdreyf/ezlimma) and [ezlimmaplot](https://github.com/jdreyf/ezlimmaplot).

# RStudio
- [RStudio](https://www.rstudio.com/) is an integrated development environment (IDE) for R  
...
## R Markdown
This report is written in [R Markdown](https://rmarkdown.rstudio.com/), which weaves together text, figures, and code. You can "Knit" R Markdown documents into HTML, PDF, Microsoft Word, or other formats by pressing the "Knit" icon above in R Studio.
...
# Example data set
We'll work with a simulated gene expression matrix with 100 genes & 9 samples, split into 3 groups. `rnorm()` randomly simulates normally distributed values. We make the 1st gene up-regulated in group 'a'.
```{r}
...
# `ezlimma` package
You can install and load `ezlimma` by following instructions at [ezlimma](https://github.com/jdreyf/ezlimma). `ezlimma`'s tutorial is presented as a *vignette*, like many R packages.
...
`toptab` is ordered by overall significance of the 3 tests. We round the number of significant digits in `toptab` to 3 with `signif`.
When we add annotation, we make sure the rows of `toptab` match the rows of `annot` using `annot[rownames(toptab),]`. We write this to CSV.
...
# `ezlimmaplot` package
We can plot `ezlimma` results with `ezlimmaplot`. We'll focus on the differential expression.

You can install and load `ezlimmaplot` by following instructions at [ezlimmaplot](https://github.com/jdreyf/ezlimmaplot). `ezlimmaplot`'s tutorial is also presented as a *vignette*.

```{r}
library(ezlimmaplot)
# browseVignettes(package="ezlimmaplot") #to see vignette
```
...
# Conclusion
Congratulations for making it this far. I hope this helped you become familiar enough with R so you can understand documentation, and hopefully apply some data analysis.