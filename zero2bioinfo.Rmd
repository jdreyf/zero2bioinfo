...
We have developed a novel method for testing causal mediation, termed High-throughput mediation analysis (`hitman`). This is applicable when treatment (or exposure) is randomized, and omics and an outcome (e.g. insulin sensitivity) are measured. Hitman then tests which analytes best mediate the effect of the treatment on the outcome. We have also extended this to testing mediation of pathways [Mesa](https://github.com/jdreyf/Mesa).

These methods have been applied to test which analytes mediate the benefit of gastric bypass on HbA1c with Dr. ME Patti (Joslin). We are intrested in applying these methods and developing new ones for new applications.
